...
Queues, in the realm of computer science, are categorized as abstract data types (ADTs) rather than fundamental data structures. 
This distinction is crucial; while concrete data types are explicit in their structure and implementation,
 ADTs like queues focus on defining a set of operations and expected behaviors.
The core principle of a queue is its order of operation, known as **First In, First Out (FIFO)**. 
This term implies that the first element added to the queue is also the first to be removed. 

It's important to note that queues are versatile; their actual implementation can be achieved using a range of different data structures
such as Arrays, LinkedLists, or even Stacks, each providing different performance characteristics 
for operations like `enqueue` (adding to the queue) and `dequeue` (removing from the queue). So, while queue itself isn't a data structure, 
it is a concept or ADT that can be realised using various data structures in programming.

We'll take a look different ways of implementing queues in Javascript
...
```
Implementing queues using linked lists in JavaScript has its own set of pros and cons compared to using arrays. Here's a breakdown:
