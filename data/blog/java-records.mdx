...
A Java Record is a special class that has some pretty powerful features. 
Records are basically immutable data carrier classes used to communicate with databases. 
Usually, in Java, we acquire to write a lot of boilerplate code like constructors, `equals()`, `hashcode()`, 
and `toString()` methods to create a simple data-carrier-class. This is often too verbose and repetitive code. 
...
Use cases and common implementations
Data transfer objects (DTO): we often write DTO classes when working 
with databases to encapsulate data and send it from one subsystem of an application to another. 
